...
BEGIN # Re-export imports from Date::Pcalc:
{
    require Exporter;
    require Date::Pcalc;
    @ISA         = qw(Exporter Date::Pcalc);
    @AUXILIARY   = qw(shift_year shift_date shift_time shift_datetime);
    @EXPORT      = @Date::Pcalc::EXPORT;
    @EXPORT_OK   = (@Date::Pcalc::EXPORT_OK,@AUXILIARY);
...
sub new
{
    ...
    bless($self, ref($class) || $class || 'Date::Pcalc');
    return $self;
}
